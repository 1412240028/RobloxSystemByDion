-- Settings.lua
-- Unified configuration for checkpoint and sprint systems
...
local Settings = {
	...
	SPRINT_SPEED_TOLERANCE = 2,

	-- Sprint Timing
	SPRINT_DEBOUNCE_TIME = 0.2,
	SPRINT_SYNC_DELAY = 0.5,

	-- Sprint Rate Limiting
	MAX_SPRINT_TOGGLES_PER_SECOND = 5,

	-- Sprint UI Config
	SPRINT_BUTTON_SIZE_PC = UDim2.new(0, 40, 0, 60),
	SPRINT_BUTTON_SIZE_MOBILE = UDim2.new(0, 60, 0, 60),
	SPRINT_BUTTON_POSITION_PC = UDim2.new(0, 30, 0.5, -30),
	SPRINT_BUTTON_POSITION_MOBILE = UDim2.new(0, 20, 0, 150),
	SPRINT_BUTTON_COLOR_OFF = Color3.fromRGB(255, 50, 50),
	SPRINT_BUTTON_COLOR_ON = Color3.fromRGB(50, 150, 255),
	SPRINT_BUTTON_CORNER_RADIUS = UDim.new(0, 8),
	SPRINT_BUTTON_STROKE_THICKNESS = 2,

	-- Sprint Animations
	SPRINT_PRESS_SCALE = 0.9,
	SPRINT_PRESS_DURATION = 0.1,
	SPRINT_RELEASE_DURATION = 0.15,
	SPRINT_STATE_CHANGE_DURATION = 0.2,

	-- Sprint Anti-Cheat
	SPRINT_HEARTBEAT_CHECK_INTERVAL = 0.5,
	SPRINT_SPEED_CHECK_TOLERANCE = 2,

	-- Sprint Platform Detection
	SPRINT_IS_MOBILE = false, -- Will be set dynamically
	SPRINT_IS_PC = true, -- Will be set dynamically

	-- Sprint Default Keybind
	SPRINT_DEFAULT_KEYBIND = Enum.KeyCode.LeftShift,

	-- Sprint Error Messages
	SPRINT_ERROR_REQUEST_FAILED = "Sprint request failed!",
	SPRINT_ERROR_CHARACTER_NOT_FOUND = "Character not found",
...
function Settings.ValidateConfig()
	...
	-- Sprint validation
	if Settings.SPRINT_SPEED <= Settings.NORMAL_SPEED then
		table.insert(errors, "SPRINT_SPEED must be greater than NORMAL_SPEED")
	end

	if Settings.MAX_ALLOWED_SPEED < Settings.SPRINT_SPEED then
...
function Settings.SetRuntimeConfig(key, value)
	if Settings[key] ~= nil then
		...
		if Settings.DEBUG_MODE then
			warn("[Settings] Runtime config updated:", key, "=", value)
		end
	else
		warn("[Settings] Invalid config key:", key)
	end
end

-- Dynamic platform detection for sprint (set on client)
if game:GetService("UserInputService").TouchEnabled then
	Settings.SPRINT_IS_MOBILE = true
	Settings.SPRINT_IS_PC = false
end
