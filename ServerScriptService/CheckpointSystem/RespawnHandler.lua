...
function HandlePlayerDeath(player, character)
    ...
    local deathCount = ServerMain.UpdatePlayerDeathCount(userId)
...
function RespawnHandler.RespawnPlayer(player, deathCount)
    ...
    local ServerMain = require(game.ServerScriptService.CheckpointSystem.ServerMain)
    local checkpointOrder = ServerMain.GetPlayerCheckpoint(userId)
    ...
    if Settings.ENABLE_DEATH_LOOP_PROTECTION and deathCount >= Settings.DEATH_LOOP_THRESHOLD then
        ...
        Log("WARN", "Death loop protection activated for %s, falling back to checkpoint %d",
            player.Name, checkpointOrder)
    end
    ...
    -- Teleport character
    RespawnHandler.TeleportCharacter(character, spawnPosition)
...
function RespawnHandler.ValidateSpawnPosition(position)
    ...
    if not raycastResult then
        Log("WARN", "No ground found at spawn position, finding nearby valid position")
    ...
    -- Check ceiling
    local ceilingRay = workspace:Raycast(groundPosition + Vector3.new(0, 1, 0), Vector3.new(0, Settings.SPAWN_RAYCAST_DISTANCE_UP, 0))
    if ceilingRay then
    ...
    for _, direction in ipairs(directions) do
        local wallRay = workspace:Raycast(groundPosition + Vector3.new(0, 1, 0), direction * Settings.SPAWN_RAYCAST_DISTANCE_WALL)
        if wallRay then
...
function RespawnHandler.FindNearbyValidPosition(centerPosition)
    ...
    while radius <= Settings.SPAWN_VALIDATION_RADIUS_END do
        for i = 1, 8 do -- 8 directions per radius
            ...
            if validatedPosition ~= centerPosition then -- Position was adjusted, meaning it's valid
    ...
    Log("WARN", "No valid position found in search radius, using original position")
